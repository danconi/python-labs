...
CN= """You would never guess what can happen when you jump into a seemingly shallow puddle at night time!
...
# CN= """You would never guess what can happen when you jump into a seemingly shallow puddle at night time!"""
# 1 collect letters into a temp variable
# 2 when word is done (check if an " " ) then:
#   a  do pig latin on the word
#    b store the pig latin in a final_variable
#     c reset the temp variable to be an "" string (blank string)

final_piglatin = ""
temp_letters = ""
for char in CN:
    if char == " " and len(temp_letters) >1:
        #do pig latin on temp_letters
        # print(temp_letters)
        first_letter = temp_letters[0]
        rest_word = temp_letters[1:]
        pig_latin = rest_word+first_letter+"ay"
        # print(pig_latin)
        final_piglatin = final_piglatin+pig_latin+" "
        temp_letters = ""
    

        
    else: #not a blank space because if " "
        temp_letters = temp_letters+char

print(final_piglatin)  
